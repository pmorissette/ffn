...
Now that we have some data, let's start manipulating it. In quantitative finance, we are often interested in the returns of a given time series. Let's calculate the returns by simply calling the {py:func}`to_returns <ffn.core.to_returns>` or {py:func}`to_log_returns <ffn.core.to_log_returns>` extension methods.
...
![png](_static/quickstart_11_0.png)
...
![png](_static/quickstart_15_0.png)

We used the {py:func}`ffn.core.plot_corr_heatmap`, which is a convenience method that simply calls ffn's {py:func}`ffn.core.plot_heatmap` with sane arguments.

Let's start looking at how all these securities performed over the period. To achieve this, we will plot rebased time series so that we can see how they each performed relative to each other.

```python
ax = data.rebase().plot(figsize=(12,5))
```

![png](_static/quickstart_17_0.png)

## Performance Measurement

For a more complete view of each asset's performance over the period, we can use the {py:func}`ffn.core.calc_stats` method which will create a {py:class}`ffn.core.GroupStats` object. A GroupStats object wraps a bunch of {py:class}`ffn.core.PerformanceStats` objects in a dict with some added convenience methods.
...
![png](_static/quickstart_21_0.png)
...
![png](_static/quickstart_26_0.png)
...
ffn also provides commonly used numerical routines and plans to add many more in the future. One can easily determine the proper weights using a mean-variance approach using the {py:func}`ffn.core.calc_mean_var_weights` function.
